...
class ServerSidePaginationExample(ExcelDownload, MainMenu, DatatableView):
    """
    ...
    def add_to_context(self, **kwargs):
        return {'description': (
            ...
            'The filter blocks on the left are applied server-side: badge '
            'counts come from GROUP BY queries and are only recalculated '
            'when the filter or search state changes.'
        )}


class ServerSideTagFilterExample(MainMenu, DatatableView):
    ...
    def add_to_context(self, **kwargs):
        return {'description': (
            'Server-side tag filter on a many-to-many column. Each tag '
            'checkbox cycles through four states: neutral, include (rows '
            'with any included tag), required (rows with every required '
            'tag), and exclude. The selections are applied as ORM queries '
            'and the badge counts come from GROUP BY queries with '
            'distinct row counts.'
        )}


class ServerSideTotalsFilterExample(MainMenu, DatatableView):
    ...
    def add_to_context(self, **kwargs):
        return {'description': (
            'Server-side totals filter. The filter block behaves like a '
            'pivot filter but the Filtered / Total columns show '
            '<code>Sum(amount)</code> per company computed with GROUP BY '
            'queries instead of row counts.'
        )}
...
class ServerSideJsonColumnExample(MainMenu, DatatableView):
    ...
    def add_to_context(self, **kwargs):
        return {'description': (
            'Server-side pivot filter on a JSON-field key. The column '
            'renders <code>options[\'newsletter\']</code> as a tick or '
            'blank in <code>row_result</code>, so the filter is given the '
            'ORM path with <code>field=\'options__newsletter\'</code> and a '
            '<code>choices=</code> map that folds <code>True</code>, '
            '<code>False</code>, and a missing key into the Yes / No labels. '
            'Selections are applied as JSON key lookups and the badge '
            'counts come from a GROUP BY on the key transform.'
        )}
